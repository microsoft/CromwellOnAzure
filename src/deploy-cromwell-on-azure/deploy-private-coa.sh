...
# and location. It creates a virtual network, then creates a VM jumpbox for deployment from within the virtual network, and executes
...
# Create the virtual network with the main address space
...
ssh -o StrictHostKeyChecking=no azureuser@$vm_public_ip "$coa_binary_path/$coa_binary \
    ...
    --DebugLogging true \
    --KubernetesServiceCidr 10.1.4.0/24 \
    --KubernetesDnsServiceIp 10.1.4.10"
